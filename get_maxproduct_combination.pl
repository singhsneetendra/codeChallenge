...
sub main {
  ...
  my $max_product    = get_max_product(@numbers_matrix);
...
sub get_max_product {
    ...
    for ( my $m = 0 ; $m <= $#matrix ; $m++ ) {
      for ( my $n = 0 ; $n <= $#matrix ; $n++ ) {
          #        print "$matrix[$m][$n] ";
        get_max_product_row_column( $m, $n, @matrix );
        #        get_max_product_diagonaly({matrix=> @matrix, numner_cordinate => ($m, $n) });
      }
      print "\n";
    }
    return $max_product ;
...
sub get_max_product_row_column {
        ...
        print "This is value for point- " . "$matrix[$a][$b]" . "\n" ;
        my $product_left = $matrix[$a][$b] * $matrix[$a-1][$b] * $matrix[$a-2][$b] * $matrix[$a-3][$b] ;
        print $product_left . "\n" ;
